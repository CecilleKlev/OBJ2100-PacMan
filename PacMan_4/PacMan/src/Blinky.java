...
public class Blinky extends Ghost {
	...
	/** 
	*  Class object representing the ghost Blinky
	*/

	public Blinky(int r, int c, int s, PacManBoard pmb) {
	...
	public void chase() {
		...
		// Find a path from own position to the Puck (the Pacman)
		boolean found = solve(row, col, prow, pcol);

		// The direction marks the first step of the path to the puck
		if (found) {
			if (col > 1) 
				if (m.A[row][col-1] == 5)
					dir = Main.LEFT;
	...
	public boolean solve(int r0, int c0, int r1, int c1)
	{
		...
		if (m.valid(r0,c0) != false)
		{
			...
			{
				// decide in which direction to search
				//the maze
